...
│                    Application Layer                        │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  AccountUseCase      TransactionUseCase              │   │
│  │  (Business Logic)    (Business Logic)                │   │
│  └──────────────┬───────────────────────┬────────────────┘  │
│                 │                       │                   │
│  ┌──────────────▼───────┐   ┌──────────▼──────────┐         │
│  │  FeeCalculator       │   │  LimitChecker       │         │
│  │  (전략 패턴)           │   │  (전략 패턴)           │         │
│  └──────────────────────┘   └─────────────────────┘         │
└─────────────────────────┬───────────────────────────────────┘
                          │
...
│                      Domain Layer                           │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  Account (순수 도메인 모델 - 인프라 의존성 Zero)            │   │
│  │  Transaction (불변 객체)                               │   │
│  │  AccountPort, TransactionPort, TimePort (인터페이스)    │   │
│  └──────────────────────────────────────────────────────┘   │
└───────────────────────────────────────────────▲─────────────┘
                                                │
                          구현 (implements)      │
...
│  ┌────────────────────────────────────────────┼─────────┐   │
│  │  AccountPersistenceAdapter (Port 구현) ────┘         │   │
│  │  TransactionPersistenceAdapter (Port 구현)           │   │
...
```
jeongminkim-backend/
├── jeongminkim-domain/          # 순수 도메인 모델
├── jeongminkim-application/     # 비즈니스 로직
├── jeongminkim-infrastructure/  # JPA/MySQL 구현
└── jeongminkim-api/            # REST API
```
...
### 1. 헥사고날 아키텍처 (Port-Adapter 패턴)
- **의존성 방향**: API → Application → Domain ← Infrastructure
- **Domain 순수성**: JPA, Spring 의존성 Zero
- **확장성**: Infrastructure만 교체하여 DB 변경 가능
